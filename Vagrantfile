...
Vagrant.configure("2") do |config|
  config.vm.define :macos, autostart: false do |macos|
    macos.vm.box = "http://files.dryga.com/boxes/osx-sierra-0.3.1.box"

    # Create a private network, which allows host-only access to the machine
    # using a specific IP. This is required to create NFS shared folder.
    macos.vm.network "private_network", ip: "192.168.99.33"
    ...
    macos.vm.synced_folder ".", "/vagrant/dotfiles",
          id: "core",
          :nfs => true,
    ...
    macos.vm.provision "shell", inline: <<-SHELL
      export DEV_PROJECTS_ROOT=/Users/vagrant/proj

      mkdir -p $DEV_PROJECTS_ROOT

      cp -R /vagrant/ $DEV_PROJECTS_ROOT

      cd $DEV_PROJECTS_ROOT/dotfiles
  ...
  config.vm.define :win10, autostart: false do |win10|
    # TODO: wire up a win10 box to test provisioning
  end
end